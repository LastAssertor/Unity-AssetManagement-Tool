    ...
    public sealed class AssetManager : MonoBehaviour {
		...
		public static AssetManager Instance {
			get {
				...
				if (m_Instance == null) {
					m_Instance = new GameObject(typeof(AssetManager).Name).AddComponent<AssetManager>();
        ...
        public AssetManager SetAssetBundleManifest(AssetBundleManifest manifest) {
            Debug.LogWarning("setup assetBundleManifest : new version ===> " + manifest.Version);
            m_AssetBundleManifest = manifest;
            return this;
        }

        public AssetManager SetAssetBundlesFolder(string assetBundlesFolder) {
            Debug.Log("setup assetBundles folder : " + PathUtility.ASSETBUNDLES_FOLDER + " ===> " + assetBundlesFolder);
		...
		public AssetManager LoadAssetAsync<T>(string assetPath, Action<T> completed = null) where T : Object {
        ...
        public AssetManager LoadAssetsAsync<T>(List<string> assetPaths, Action<List<T>> completed = null) where T : Object {
		...
		public AssetManager UnloadAssetAsync(string assetPath, bool unloadUnusedAssets = false, Action completed = null) {
			return UnloadAssetsAsync (new List<string>{ assetPath }, unloadUnusedAssets, completed);
		}

        public AssetManager UnloadAssetsAsync(List<string> assetPaths, bool unloadUnusedAssets = false, Action completed = null) {
            ActionSequence.Enqueue(() => InternalUnloadAssets(assetPaths, unloadUnusedAssets, completed), true);
            return this;
        }

        public AssetManager UnloadAllAssetsAsync(bool unloadUnusedAssets = false, Action completed = null) {
		...
		/// </summary>
        public AssetManager Shutdown(bool unloadUnusedAssets = false) {
